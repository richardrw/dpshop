...
class Get_Tag_Addr(object):
    ...
    #     self.dp = self.client['dp']
    #     self.tag1_url_db = self.dp['tag1_url_db']                            #存储从start_url中成功爬取到的tag1_url
    #     self.tag2_url_db = self.dp['tag2_url_db']                            #存储从tag1_url中成功爬取到的tag2_url
    #     self.crawly_tag1_url_bad = self.dp['crawly_tag1_url_bad']            #存储爬取失败的tag1_url
    #     self.addr1_url_db = self.dp['addr1_url_db']                            #存储从tag2_url中成功爬取到的addr1_url
    #     self.crawly_tag2_url_bad = self.dp['crawly_tag2_url_bad']            #存储爬取失败的tag2_url
    #     self.addr2_url_db = self.dp['addr2_url_db']                            #存储从addr1_url中成功爬取到的addr2_url
    #     self.crawly_addr1_url_bad = self.dp['crawly_addr1_url_bad']        #存储爬取失败的addr1_url
...
#     timeout = TIMEOUT
#     addr2_list = []
#     tag2 = addr1_url_db.find_one({'url': addr1_url})['tag']
#     addr1 = addr1_url_db.find_one({'url': addr1_url})['addr']
#     addr2_list.append(addr1)
#     try:
#         r = get(addr1_url, proxies=proxies, timeout=timeout)
#         tree = etree.HTML(r.text)
#         addr2_items = tree.xpath('//div[@id="region-nav-sub"]/a')
#         if len(addr2_items) == 0:
#             addr2_list.append('not_sub')
#             addr2 = {'tag': tag2, 'addr': addr2_list, 'url': addr1_url, 'status': 'addr1_not_sub'}
#             addr2_url_db.insert_one(addr2)
#         else:
#             for i in addr2_items:
#                 addr = [i.getchildren()[0].text]
#                 url = i.attrib['href']
